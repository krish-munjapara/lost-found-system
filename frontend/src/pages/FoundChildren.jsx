/**
 * Guardian-Link — Found Children Page
 * Lists all found children using the ChildCard component.
 */
...
const FoundChildren = () => {
  ...
  const loadChildren = async () => {
    try {
      const data = await childrenApi.getFound();
      setChildren(data);
    } catch (err) {
      console.error('Failed to load found children:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Layout>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        ...
        <Link to="/report-found" className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg shadow-sm hover:shadow transition-all text-sm flex items-center gap-2">
          + Report Found
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredChildren.map((child, idx) => (
            <ChildCard key={child.id || idx} child={child} type="found" index={idx} />
          ))}
        </div>
      ) : (
        <div className="flex flex-col items-center justify-center p-16 text-center border-2 border-dashed border-slate-200 rounded-2xl bg-white/50">
          ...
          <h3 className="text-lg font-bold text-slate-700 mb-1">No found children</h3>
